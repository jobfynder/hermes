...
class FallbackCompletionTests(unittest.TestCase):
 def test_partial_job_stays_in_review(self):
  ...
  with patch('app.email_parsing.llm_fallback.HERMES_EMAIL_LLM_FALLBACK_ENABLED',True), patch('app.email_parsing.llm_fallback.run_llm_fallback',return_value={'used':True,'extracted':{'location':'Remote'}}) as call:
 ...
 def test_partial_hotlist_stays_in_review(self):
  with patch('app.email_parsing.llm_fallback.HERMES_EMAIL_LLM_FALLBACK_ENABLED',True), patch('app.email_parsing.llm_fallback.run_llm_fallback',return_value={'used':True,'extracted':{'consultants':[{'candidate_name':'Example'}]}}):
   result,_=apply_hotlist_fallback('Incomplete hotlist',{'confidence':0.2})
  self.assertTrue(result['requires_review'])
 def test_empty_input_never_calls_model(self):
  with patch('app.email_parsing.llm_fallback.HERMES_EMAIL_LLM_FALLBACK_ENABLED',True), patch('app.email_parsing.llm_fallback.run_llm_fallback') as call:
